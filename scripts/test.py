...
def test_oclassi():
    ...
    oclassi = OnlineEMGClassifier(classi, sensor.window_size, sensor.window_increment, odh, ["MAV"], std_out=False)
...
def test_online_rw():
    ...
    with open(paths.live_data+"EMG.csv", "r") as csv:
        while True:
            ...
            print(f"Time to decode {len(newlines)} newlines: {time.time()-timestamp:.5f} s")
            ...
            stamps = data[:, 0]
            valid_samples = np.squeeze(np.argwhere(np.logical_and(stamps > last_timestamp, stamps <= timestamp)))
