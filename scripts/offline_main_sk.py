...
def __main():
    ...
    sensor = EmgSensor(g.SENSOR, window_size_ms=150, window_inc_ms=20, majority_vote_ms=0)

    paths = NfcPaths(f"data/{sensor.get_name()}")
    # paths.set_trial_number(-1)
    paths.set_trial_number(2)
    paths.gestures = "data/gestures/"

    classes = utils.get_cid_from_gid(paths.gestures, paths.train, g.FUNCTIONAL_SET)
    ...
    results = om.extract_offline_metrics(metrics, test_labels, test_preds, null_label=idle_cid)
    ...
    test_gesture_names = utils.get_name_from_gid(paths.gestures, paths.train, g.FUNCTIONAL_SET)
