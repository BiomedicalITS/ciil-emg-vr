...
namespace Unity.VRTemplate
{
    ...
    public class SmartFollowVector3TweenableVariable : Vector3TweenableVariable
    {
        ...
        /// <returns>Returns true if the squared distance between the current and new targets is within the allowed threshold, false otherwise.</returns>
        ...
        [BurstCompile]
#endif
        static void ComputeNewTweenTarget(in float3 currentValue, in float3 targetValue, in float sqrMaxDistanceAllowed, in float deltaTime, in float lowerSpeed, in float upperSpeed, out float newTweenTarget)
        {
        ...
        [BurstCompile]
#endif
        static bool IsNewTargetWithinThreshold(in float3 currentValue, in float3 targetValue, in float minDistanceAllowed, in float maxDistanceAllowed, in float timeSinceLastUpdate, in float minToMaxDelaySeconds)
        {
